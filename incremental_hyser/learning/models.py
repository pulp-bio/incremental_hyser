...
class TinierNet8(nn.Module):
    ...
    def __init__(self,
        ...
        self.b0_conv = nn.Conv1d(self.num_ch_in, 32, 3, padding='same', bias=False)
        ...
        self.b2_conv = nn.Conv1d(32, 32, 3, padding='same', bias=False)
        self.b2_bn = nn.BatchNorm1d(32)
        self.b2_relu = nn.ReLU()
        self.b2_pool = nn.AvgPool1d(2, stride=2)

        self.b3_conv = nn.Conv1d(32, 32, 3, padding='same', bias=False)
        self.b3_bn = nn.BatchNorm1d(32)
        self.b3_relu = nn.ReLU()
        self.b3_pool = nn.AvgPool1d(2, stride=2)

        self.b4_conv = nn.Conv1d(32, 64, 3, padding='same', bias=False)
        self.b4_bn = nn.BatchNorm1d(64)
        self.b4_relu = nn.ReLU()
        self.b4_pool = nn.AvgPool1d(2, stride=2)

        self.b5_conv = nn.Conv1d(64, 64, 3, padding='same', bias=False)
        self.b5_bn = nn.BatchNorm1d(64)
        self.b5_relu = nn.ReLU()
        self.b5_pool = nn.AvgPool1d(2, stride=2)

        self.b6_conv = nn.Conv1d(64, 64, 3, padding='same', bias=False)
        self.b6_bn = nn.BatchNorm1d(64)
        self.b6_relu = nn.ReLU()
        self.b6_pool = nn.AvgPool1d(2, stride=2)

        self.b7_conv = nn.Conv1d(64, 64, 3, padding='same', bias=False)
        self.b7_bn = nn.BatchNorm1d(64)
        self.b7_relu = nn.ReLU()
        self.b7_pool = nn.AvgPool1d(2, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.b0_pool(self.b0_relu(self.b0_bn(self.b0_conv(x))))
        x = self.b1_pool(self.b1_relu(self.b1_bn(self.b1_conv(x))))
        x = self.b2_pool(self.b2_relu(self.b2_bn(self.b2_conv(x))))
        x = self.b3_pool(self.b3_relu(self.b3_bn(self.b3_conv(x))))
        x = self.b4_pool(self.b4_relu(self.b4_bn(self.b4_conv(x))))
        x = self.b5_pool(self.b5_relu(self.b5_bn(self.b5_conv(x))))
        x = self.b6_pool(self.b6_relu(self.b6_bn(self.b6_conv(x))))
        x = self.b7_pool(self.b7_relu(self.b7_bn(self.b7_conv(x))))
...
def main() -> None:
    ...
    tiniernet8 = TinierNet8(num_ch_in, num_ch_out)
    tiniernet8.eval()

    verbose = 2
    summarize(tiniernet8, num_ch_in, num_samples_in, verbose=verbose)
